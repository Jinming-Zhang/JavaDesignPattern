package stradegy.utoronto.utsc.grader.grades;

import stradegy.utoronto.utsc.grader.grades.InvalidGradeException;
...
/*	private String type;
	...
	*//**
	 ...
	 * 
	 * @param score
	 *            the numeric value of the new <code>Grade</code>, must be in
	 *            the range [0, 100].
	...
	*//**
	 ...
	 * 
	 * @param grade
	 *            the value of the new <code>LetterGrade</code>
	 * @throws InvalidGradeException
	...
	public Grade(String grade) throws InvalidGradeException {
		if (grade.length() == 1 || grade.length() == 2) {
			...
			if (Arrays.asList(VALID_LETTERS).contains(letter) && Arrays.asList(VALID_SIGNS).contains(sign)) {
				this.type = "letter";
				this.letter = letter;
				this.sign = sign;
				return;
